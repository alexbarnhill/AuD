...
/**
 * This class contains the front-end (GUI) for Lights Out. <br/>
 * For a general description of the game please refer to: {@link <a href="https://en.wikipedia.org/wiki/Lights_Out_%28game%29">Lights Out (game)</a>}. <br/>
 * <br/>
 * 
 * @author John Doe
 * @version 1.1, 12/01/17
 */
public final class LightsOutGUI {
	 ...
	 * 
	 * @param args
	 *            optional command line arguments, denoting the size of the game grid (2 <= rows, cols <= 42) and the densities of lights and blinds (0 <= light, mask < 1)
	 */
	...
	private static final class LightsOutInner extends JFrame implements ActionListener, Runnable {
		...
		private void startNewGame() {
			...
			for (int p = 0; p < rows * cols; p++) {
				if (Math.random() < lightDensity) {
					state = BitOps.set(state, p);
				}
				if (Math.random() < maskDensity) {
					mask = BitOps.set(mask, p);
				}
			}
			lightsOut = new LightsOut(cols, rows, state, mask);
		...
		private void paintMatrix() {
			for (int r = 0; r < rows; r++) {
				for (int c = 0; c < cols; c++) {
					if (BitOps.isSet(mask, r * cols + c)) {
						matrixGUI[r][c].setBackground(BLOCKED);
						matrixGUI[r][c].setBorder(LOWERED);
					} else if (BitOps.isSet(lightsOut.getState(), r * cols + c)) {
		...
		public void run() {
			...
			Integer[] moves = movesMerker == null ? null : movesMerker.gibtMirAlle();
